...
static uint8 *createPaddedCopyWithLength(const uint8 *b, uint32 *l) {
  ...
  if (newLen448 <= len) newLen448 += 64;

  *l = newLen448 + 8;
  if ((ret = (uint8 *)malloc(sizeof(uint8) * *l)) == NULL) return NULL;

  if (b != NULL) memcpy(ret, b, sizeof(uint8) * len);

  /* pad */
  ret[len] = 0x80;
  for (q = len + 1; q < newLen448; q++) ret[q] = 0x00;
...
static int calculateDigestFromBuffer(const uint8 *b, uint32 len,
                                     uint8 sum[16]) {
  ...
  if ((input = createPaddedCopyWithLength(b, &l)) == NULL) return 0;

  for (i = 0;;) {
    if ((newI = i + 16 * 4) > l) break;
    k = i + 3;
    for (j = 0; j < 16; j++) {
      wbp = (workBuff + j);
      *wbp = input[k--];
 ...
 *				  int pg_md5_hash(const void *buff, size_t len,
 *char *hexsum)
 *
 ...
 *						  hexadecimal digits.  an MD5 sum
 *is 16 bytes long. each byte is represented by two heaxadecimal characters. you
 *thus need to provide an array of 33 characters, including the trailing '\0'.
 *
...
bool pg_md5_hash(const void *buff, size_t len, char *hexsum) {
  ...
  if (!calculateDigestFromBuffer(buff, len, sum)) return false;
...
bool pg_md5_binary(const void *buff, size_t len, void *outbuf) {
  if (!calculateDigestFromBuffer(buff, len, outbuf)) return false;
  return true;
}
